...
export default function PrebodasPage() {
  ...
  const [prebodasGallery, setPrebodasGallery] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`${API}/servicios/catalogo`)
      .then(res => res.json())
      .then(data => {
        const filtered = data.filter((p: any) => p.categoria?.toLowerCase().includes('pre-boda'));
